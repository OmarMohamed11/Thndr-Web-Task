...
const alertVariants = cva(
    "relative w-full rounded-lg border px-4 py-3 text-sm backdrop-blur-sm shadow-lg transition-all duration-300 [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-foreground [&>svg~*]:pl-7",
    {
        variants: {
            variant: {
                default:
...
>(({ className, ...props }, ref) => (
    <div
        ref={ref}
        className={cn(
            "text-sm text-slate-300 [&_p]:leading-relaxed",
            className
        )}
        {...props}
    />
